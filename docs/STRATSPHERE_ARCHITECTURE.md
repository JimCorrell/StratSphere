...
CREATE TABLE public.leagues (
  ...
  status           VARCHAR(20) DEFAULT 'Setup',  -- enum: Setup | Active | Completed
...
CREATE TABLE public.teams (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  league_id     UUID REFERENCES leagues(id),
  user_id       UUID REFERENCES asp_net_users(id),   -- NULL until a manager claims the team
  name          VARCHAR(100),
  city          VARCHAR(100),
  abbreviation  CHAR(3),
...
-- Soft-delete via DroppedAt: a card can be acquired, dropped, and re-acquired within a season.
-- The partial unique index prevents a card from being on two teams simultaneously.
CREATE TABLE public.roster_slots (
  ...
  dropped_at   TIMESTAMPTZ            -- NULL = currently active
);

-- Partial unique index: a card can only be on one team per season at a time
CREATE UNIQUE INDEX IX_roster_slots_active_card_season
  ON public.roster_slots (season_id, card_id)
...
| Roster management — add/drop cards per season (soft-delete) | ✅ Done |
...
| Sim stat row initialization on roster add | 🔲 Next |
